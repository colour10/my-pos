    ...
    <div class="container-fluid head-table">
        ...
        <div class="container-fluid">
                ...
                <form action="{{ route('agents.search') }}" method="get" id="form-search">
                    ...
                    <div class="form-select">
                        ...
                        <select class="form-control" name="status">
                            <option value="" @if ($request->get('status') === NULL) selected="selected"@endif>全部
                            </option>
                            <option value="0" @if ($request->get('status') == '0') selected="selected"@endif>待审核
                            </option>
                            <option value="1" @if ($request->get('status') == '1') selected="selected"@endif>审核通过
                            </option>
                            <option value="2" @if ($request->get('status') == '2') selected="selected"@endif>未通过
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="form-select">
                        <br>
                        <label class="form-select-title">余额筛选：</label>
                        <select class="form-control" name="amount">
                            <option value="" @if ($request->get('amount') === NULL) selected="selected"@endif>全部
                            </option>
                            <option value="1" @if ($request->get('amount') == '1') selected="selected"@endif>大于0
                            </option>
                            <option value="2" @if ($request->get('status') == '2') selected="selected"@endif>等于0
                            </option>
                        </select>
                    </div>
            ...
            <div class="pull-right text-right" style="width:16%">
                <a href="/admin/agents/export?<?php echo $_SERVER['QUERY_STRING']; ?>"
                        ...
                        <tr>
                            ...
                            <td>
                                <a class="btn btn-small btn-update"
                                   onclick="agent_show({{ json_encode($agent) }});return false;">详情</a>
                                @if ($agent->status == 0)
                                    <a class="btn btn-small btn-edit"
                                       onclick="agent_review({{ $agent->id }});return false;">审核</a>
                                @endif
                                <a href="{{ route('agents.edit', ['id' => $agent->id]) }}"
                                   class="btn btn-small btn-check">变更</a>
                            </td>
                        </tr>
                    @endforeach
        ...
        <!-- 分页Start -->
            <div class="paging">
                ...
                @if ($controller_action['action'] == 'search')
                    {!! $agents->appends($request->all())->render() !!}　<p class="pull-right" style="padding:6px 12px;">
                        共有{{ $agents->total() }}条记录</p>
                @else
                    {!! $agents->links() !!}　<p class="pull-right" style="padding:6px 12px;">共有{{ $agents->total() }}
                        条记录</p>
                @endif
